...
import { LoadingSpinner, SkillsCard } from "@/components";
...
export default function DevelopersPage() {
  ...
  const fetchDevelopers = async ({ pageParam = 1 }) => {
    const { data } = await customFetch(`/skills?page=${pageParam}&limit=6`);
    return data;
  };
      ...
      <div className="container px-4 sm:px-6 lg:px-8">
        {status === "pending" ? (
          ...
          <div className="space-y-12">
                ...
                <div className="relativef md:flex items-center justify-between">
                  ...
                  <div className="flex gap-4 items-center mb-2">
                    <button
                      ...
                      className="lg:hidden mx-auto mt-2 md:mt-0 px-2 py-1 bg-primary rounded-lg font-bold text-white"
                    >
                      Let's Chat
                    </button>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 transition-all duration-500 ease-in-out">
                  {data.pages.map((page) =>
                    page.skills.map((skill) => (
                      ...
                      >
                        <SkillsCard skill={skill} layout={layout} />
                      </div>
                    ))
                  )}
                </div>
            ...
            {!hasNextPage && data.pages[0].skills.length > 0 && (
